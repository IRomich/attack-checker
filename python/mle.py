...
def distribution_estimate(data, distributions, verb_level=13, f_plot_pdf=True):
    ...
    return dist_names[xranged_indexes[-1]]
    sep = 20 * "----"
	print("{}\n  Distribution\tLikelihood\t\t\tParameters\n{}".format(sep, sep))
	for i in xrange(1, verb_level+1):
		print("%14s: %10.4f   %s" % (dist_names[xranged_indexes[-i]], llvalue[xranged_indexes[-i]], parameters[xranged_indexes[-i]]))
	print(sep)
...
def hurst_RS(ts, maxlag='auto', f_plot=True):
	...
	for lag in lags:
		...
		for i in xrange(int(ts_length / lag)):
			...
			# Maximum xrange in block with centred values
		...
		if ts_length % lag != 0:
			...
			R.append(max(np.cumsum(ts_centred)) - min(np.cumsum(ts_centred)))
			S.append(np.std(ts[i*lag:(i+1)*lag]))
		# Mean normed xrange for current lag size
	...
	# Plot results
	if f_plot:
		# R/S-statistics on full xrange of possible lags
		RS_full = []
...
def hurst_var(ts, maxlag='auto', f_plot=True):
	...
	# Plot results
	if f_plot:
		# Variances on full xrange of possible lags
		var_full = []
		for lag in xrange(2, int(ts_length / 2)):
			av_series = []
			for i in xrange(int(ts_length / lag)):
